    ...
    public class PlaneField
    {
        ...
        private static IEnumerator DrawSelfAndAdjacentTiles(Action onGridGenerationCompletion, SpriteRenderer tilePrefab)
        {
            ...
            for (int x = 0; x < grid.GetLength(0); x++)
            {
                ...
                for (int y = 1; y <= x; y++)
                    PlaneFieldRenderer.CreateVisualTile(grid[x - y, Mathf.Clamp(y, 0, grid.GetLength(1) - 1)], tilePrefab);
            ...
            for (int y = 1; y < grid.GetLength(1); y++)
            {
                for (int x = grid.GetLength(0) - 1; x >= 0; x--)
                    PlaneFieldRenderer.CreateVisualTile(grid[x, Mathf.Clamp(y + grid.GetLength(0) - 1 - x, 0, grid.GetLength(1) - 1)], tilePrefab);
